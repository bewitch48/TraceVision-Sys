...
class Model_VSN(BaseModel):
    def __init__(self, opt):
        ...
        if opt['dist'] and self.device.type == 'cuda':
        ...
        self.pipe = None
        self.pipe_control = None
        self.pipe_sdxl = None
        self.pipe_repaint = None
        self.scheduler = None
        ...
        if self.is_train:
            ...
            if self.mode == "image":
                for k, v in self.netG.named_parameters():
                    # Handle both DataParallel (module.xxx) and non-parallel (xxx) cases
                    if ((k.startswith('module.irn') or k.startswith('module.pm') or 
                         k.startswith('irn') or k.startswith('pm')) and v.requires_grad): 
                ...
                for k, v in self.netG.named_parameters():
                    # Handle both DataParallel (module.xxx) and non-parallel (xxx) cases
                    if ((k.startswith('module.bitencoder') or k.startswith('module.bitdecoder') or 
                         k.startswith('bitencoder') or k.startswith('bitdecoder')) and v.requires_grad):
                        optim_params.append(v)
                    else:
                        if self.rank <= 0:
                            logger.warning('Params [{:s}] will not optimize.'.format(k))
    ...
    def init_hidden_state(self, z):
        ...
        for _ in range(self.opt_net['block_num_rbm']):
            h_t.append(torch.zeros([b, c, h, w]).to(z.device))
            c_t.append(torch.zeros([b, c, h, w]).to(z.device))
        memory = torch.zeros([b, c, h, w]).to(z.device)
    ...
    def optimize_parameters(self, current_step):
        ...
        if degrade_shuffle:
            ...
            if choice == 0:
                ...
                torchnoise = torch.from_numpy(noise).to(y_forw.device).float()
                y_forw = y_forw + torchnoise

            elif choice == 1:
                NL = int(np.random.randint(70,95))
                self.DiffJPEG = DiffJPEG(differentiable=True, quality=int(NL)).to(y_forw.device)
    ...
    def test(self, image_id):
        ...
        with torch.no_grad():
            ...
            if self.opt['hide']:
                ...
                message = torch.tensor(self.msg_list[image_id]).unsqueeze(0).to(self.device)
            ...
            if add_sdinpaint:
                ...
                y_forw = torch.stack(forw_list, dim=0).float().to(self.device)

            if add_controlnet:
                if self.pipe_control is None:
            ...
            if degrade_shuffle:
                ...
                if add_noise:
                    ...
                    self.DiffJPEG = DiffJPEG(differentiable=True, quality=int(Q)).to(y_forw.device)
                    y_forw = self.DiffJPEG(y_forw)
    ...
    def image_recovery(self, number):
        ...
        with torch.no_grad():
            ...
            # 保存供外部诊断
            self.rec_loc = rec_loc.detach()
            self.template = template.detach()
