...
# 全局缓存 SD-Inpainting Pipeline，避免每次调用重新加载模型
_inpaint_pipe = None

def _get_inpaint_pipe():
    """懒加载 Stable Diffusion Inpainting Pipeline（全局单例）"""
    ...
    if _inpaint_pipe is None:
        ...
        try:
            ...
            print(f"[ERROR] Failed to load SD-Inpainting model: {e}")
            raise RuntimeError(f"SD-Inpainting 模型加载失败: {e}")
    return _inpaint_pipe


def image_editing(image_numpy, mask_image, prompt, num_inference_steps=30, guidance_scale=12.0, strength=0.85):
    ...
    target_size = (512, 512)
    image_init = pil_image.convert("RGB").resize(target_size)
    mask_image = mask_image.resize(target_size)
    
    w, h = target_size
    
    # 每次调用使用不同的随机种子，确保不同提示词产生不同结果
